...
import { getDiscordMemberCount, getDiscordProfiles } from "@/lib/discord.functions";
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "667 clan resmî sitesi. Karanlık, sessiz ve hızlı.",
      },
...
function ProfileCards() {
  const checkProfiles = useServerFn(getDiscordProfiles);
  const { data } = useQuery({
    queryKey: ["dsc-readme-availability"],
    queryFn: () => checkProfiles({ data: { ids: PROFILES.map((p) => p.id) } }),
    staleTime: 5 * 60 * 1000,
  });

  const visible = PROFILES.filter((profile) => data?.find((p) => p.id === profile.id)?.available);
  ...
  return (
    <div className="mt-6 flex max-w-3xl flex-wrap items-center justify-center gap-3">
      {visible.map((profile) => (
        ...
        >
          <img
            src={`https://dsc-readme.tsuni.dev/api/user/${profile.id}?${profile.params}`}
